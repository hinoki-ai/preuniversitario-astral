...
export default function ZoomJoinClient({ initialMeetingNumber, initialPasscode }: { initialMeetingNumber?: string, initialPasscode?: string }) {
  const { user } = useUser()
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Vendor + SDK scripts */}
      <Script src={`https://source.zoom.us/${ZOOM_VERSION}/lib/vendor/react.min.js`} strategy="afterInteractive" onLoad={onZoomSdkReady} />
      <Script src={`https://source.zoom.us/${ZOOM_VERSION}/lib/vendor/react-dom.min.js`} strategy="afterInteractive" onLoad={onZoomSdkReady} />
      <Script src={`https://source.zoom.us/${ZOOM_VERSION}/zoom-meeting.min.js`} strategy="afterInteractive" onLoad={onZoomSdkReady} />

      <div className="rounded-lg border bg-card p-6 space-y-4">
        <div>
          <h2 className="text-lg font-semibold">Unirse a Clase en Vivo (Zoom)</h2>
          <p className="text-sm text-muted-foreground">Ingresa el ID y código entregados por el profesor. La videollamada se abrirá aquí mismo.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2">
            <Label htmlFor="meetingNumber">ID de reunión</Label>
            <Input id="meetingNumber" placeholder="e.g. 123 4567 8901" value={meetingNumber} onChange={(e) => setMeetingNumber(e.target.value.replace(/\s/g, ""))} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="passcode">Código de acceso</Label>
            <Input id="passcode" placeholder="Código" value={passcode} onChange={(e) => setPasscode(e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="displayName">Tu nombre</Label>
            <Input id="displayName" placeholder="Nombre" value={displayName} onChange={(e) => setDisplayName(e.target.value)} />
          </div>
        </div>
